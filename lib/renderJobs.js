...
export async function createJob({ apiKey, userId, spec }) {
    const pool = await db();
    ...
    await pool.query(
        `INSERT INTO render_jobs (id, user_id, key_hash, superb_key, spec_json) VALUES ($1, $2, $3, $4, $5)`,
        [id, userId || null, hashKey(apiKey), apiKey, JSON.stringify(spec)],
    );
...
async function groundPrompt(spec, apiKey) {
    const images = [];
    try {
    ...
    if (String(spec.prompt || '').length > 350) return { prompt: spec.prompt, visionUsed: false };
    try {
        ...
        if (!response.ok) return { prompt: spec.prompt, visionUsed: false };
        ...
        return { prompt: spec.prompt, visionUsed: false };
    }
}
...
async function processQueuedJob(id) {
    ...
    const grounded = await groundPrompt(spec, row.superb_key);
