...
// When the provider's own wallet is empty, EVERY render fails identically.
// Creating more jobs just fills the board with the same red card, so the API
// refuses up front — cheaply, from recent evidence, with no upstream call.
const CREDIT_BLOCK_WINDOW_MINUTES = 10;

export async function providerCreditBlock() {
    const pool = await db();
    if (!pool) return null;
    try {
        const result = await pool.query(`
            SELECT error, created_at FROM render_jobs
            WHERE status = 'failed'
              AND created_at > now() - interval '${CREDIT_BLOCK_WINDOW_MINUTES} minutes'
            ORDER BY created_at DESC LIMIT 20
        `);
        const hit = result.rows.find((row) => isProviderOutOfCredit(row.error));
        if (!hit) return null;
        // A success since then means it was topped up — clear the block.
        const ok = await pool.query(`
            SELECT 1 FROM render_jobs
            WHERE status = 'done' AND created_at > $1 LIMIT 1
        `, [hit.created_at]);
        if (ok.rowCount > 0) return null;
        return String(hit.error);
    } catch {
        return null;
    }
}
...
// Removing a card from the board must remove the row too, or hydrate() puts
// it straight back. Scoped to the key that owns the job — one user can never
// delete another's history.
export async function deleteJob(id, apiKey) {
    const pool = await db();
    if (!pool) return false;
    const result = await pool.query(
        'DELETE FROM render_jobs WHERE id = $1 AND key_hash = $2 RETURNING id',
        [id, hashKey(apiKey)],
    );
